        ...
        <Nav>
          ...
          {isLogin && (
            <div style={{margin:'5px'}}>
              {isPremiumEligible && !isActive && (<Button variant='success' onClick={activePremiumHandler}>Activate Premium</Button>)}
              ...
              {isActive && <Button onClick={darkMode ? removeDarkMode : darkModeHandler}>{darkMode ? 'Light Mode':'Dark Mode'}</Button>}{' '}
              {isActive &&  <Button onClick={downloadExpense} >Download Expenses</Button>}
            </div>
            )
          }
